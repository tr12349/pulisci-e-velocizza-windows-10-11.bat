...
:: FIX PER L'ORARIO: Gestione robusta dello spazio vuoto e degli zeri iniziali per evitare crash matematici ottali
set "p_time_end=%TIME%"
if "%p_time_end:~0,1%"==" " set "p_time_end=0%p_time_end:~1%"
for /f "tokens=1-4 delims=:.," %%a in ("%p_time_end%") do (
    set "E_HH=%%a" & set "E_MM=%%b" & set "E_SS=%%c"
)
for /f "tokens=* delims=0" %%a in ("%E_HH%") do set "E_HH=%%a"
for /f "tokens=* delims=0" %%a in ("%E_MM%") do set "E_MM=%%a"
for /f "tokens=* delims=0" %%a in ("%E_SS%") do set "E_SS=%%a"
if "%E_HH%"=="" set "E_HH=0"
if "%E_MM%"=="" set "E_MM=0"
if "%E_SS%"=="" set "E_SS=0"
